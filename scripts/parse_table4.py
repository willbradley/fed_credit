#!/usr/bin/env python3
"""
Parser for Federal Credit Supplement Table 4: Loan Guarantee Programs
...
def _parse_from_df(df, cohort_year=None, known_bureaus=None):
    """Parse Table 4 data from a DataFrame."""
    ...
    for idx in range(2, len(df)):
        ...
        if is_program:
            ...
            program_name = program_name.rstrip('0123456789 ')
    ...
    return {
        'metadata': {
            ...
            'table': 'Table 4: Loan Guarantee Programs - Subsidy Estimates and Loan Characteristics',
            'cohort_year': cohort_year,
            'units': {
                'rates': 'percent',
                'maturity': 'years',
...
def parse_table4(excel_path: str, sheet_name: str = 'Table 4',
                 engine: str = None) -> dict:
    """Parse Table 4 from the Federal Credit Supplement Excel file."""
    kwargs = {'header': None}
    if engine:
...
def main():
    ...
    output_path = project_root / 'data' / 'processed' / 'table4_loan_guarantee_characteristics.json'

    data = parse_table4(str(excel_path))

    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, 'w') as f:
        json.dump(data, f, indent=2)

    print(f"Parsed {len(data['programs'])} loan guarantee programs with characteristics")
