#!/usr/bin/env python3
"""
Parser for Federal Credit Supplement Table 10: Loan Guarantee Programs
...
def _parse_from_df(df, known_bureaus=None):
    """Parse Table 10 data from a DataFrame."""
    ...
    return {
        'metadata': {
            ...
            'table': 'Table 10: Loan Guarantee Programs - Disbursement Schedules',
...
def parse_table10(excel_path: str, sheet_name: str = 'Table 10',
                  engine: str = None) -> dict:
    """Parse Table 10 from the Federal Credit Supplement Excel file."""
    kwargs = {'header': None}
    if engine:
...
def main():
    ...
    output_path = project_root / 'data' / 'processed' / 'table10_loan_guarantee_disbursement_schedules.json'

    data = parse_table10(str(excel_path))

    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, 'w') as f:
        json.dump(data, f, indent=2)

    print(f"Parsed {len(data['programs'])} loan guarantee disbursement schedules")
