#!/usr/bin/env python3
"""
Parser for Federal Credit Supplement Table 5: Direct Loan Programs
...
def _parse_from_df(df, cohort_year=None, known_bureaus=None):
    """Parse Table 5 data from a DataFrame."""
    ...
    return {
        'metadata': {
            ...
            'table': 'Table 5: Direct Loan Programs - Subsidy Estimates and Loan Characteristics',
            'cohort_year': cohort_year,
            'units': {
                'rates': 'percent',
                'maturity': 'years',
...
def parse_table5(excel_path: str, sheet_name: str = 'Table 5',
                 engine: str = None) -> dict:
    """Parse Table 5 from the Federal Credit Supplement Excel file."""
    kwargs = {'header': None}
    if engine:
...
def main():
    ...
    output_path = project_root / 'data' / 'processed' / 'table5_direct_loan_characteristics_2026.json'

    data = parse_table5(str(excel_path))

    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, 'w') as f:
        json.dump(data, f, indent=2)

    print(f"Parsed {len(data['programs'])} direct loan programs (budget-year cohort)")
