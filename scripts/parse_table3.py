#!/usr/bin/env python3
"""
Parser for Federal Credit Supplement Table 3: Direct Loan Programs
Subsidy Estimates and Loan Characteristics (prior-year cohort).
"""
...
def _parse_from_df(df, cohort_year=None, known_bureaus=None):
    """Parse Table 3 data from a DataFrame."""
    ...
    return {
        'metadata': {
            'source': f'Federal Credit Supplement, Budget of the U.S. Government, FY {cohort_year + 1 if cohort_year else ""}',
            'table': 'Table 3: Direct Loan Programs - Subsidy Estimates and Loan Characteristics',
            'cohort_year': cohort_year,
            'units': {
                'rates': 'percent',
                'maturity': 'years',
...
def parse_table3(excel_path: str, sheet_name: str = 'Table 3',
                 engine: str = None) -> dict:
    """Parse Table 3 from the Federal Credit Supplement Excel file."""
    kwargs = {'header': None}
    if engine:
...
def main():
    ...
    output_path = project_root / 'data' / 'processed' / 'table3_direct_loan_characteristics.json'

    data = parse_table3(str(excel_path))

    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, 'w') as f:
        json.dump(data, f, indent=2)

    print(f"Parsed {len(data['programs'])} direct loan programs with characteristics")
