#!/usr/bin/env python3
"""
Parser for Federal Credit Supplement Table 7: Direct Loan Programs
...
def _parse_from_df(df, known_bureaus=None):
    """Parse Table 7 data from a DataFrame."""
    ...
    return {
        'metadata': {
            ...
            'table': 'Table 7: Direct Loan Programs - Subsidy Reestimates',
...
def parse_table7(excel_path: str, sheet_name: str = 'Table 7',
                 engine: str = None) -> dict:
    """Parse Table 7 from the Federal Credit Supplement Excel file."""
    kwargs = {'header': None}
    if engine:
...
def main():
    ...
    output_path = project_root / 'data' / 'processed' / 'table7_direct_loan_reestimates.json'

    data = parse_table7(str(excel_path))
    ...
    print(f"Parsed {len(data['programs'])} direct loan programs with {total_cohorts} cohort-year records")
