#!/usr/bin/env python3
"""
Parser for Federal Credit Supplement Table 1: Direct Loan Programs
...
# Master set of known bureau names across all FCS years
KNOWN_BUREAUS = {
    'Farm Service Agency',
...
def _detect_cohort_years(df):
    """Detect the two cohort years from header rows of Tables 1/2."""
...
def _parse_from_df(df, cohort_years=None, known_bureaus=None):
    """Parse Table 1 data from a DataFrame.

    Args:
        df: pandas DataFrame loaded from the Excel sheet (header=None)
        cohort_years: tuple of (prior_year, budget_year), auto-detected if None
        known_bureaus: set of bureau names, uses default if None
    """
    ...
    for idx in range(2, len(df)):
        ...
        if is_program:
            program_name = col0.split(':')[0].strip()
            program_name = program_name.rstrip('.')
            if program_name.startswith('Stafford'):
                program_name = 'Stafford'
    ...
    return {
        'metadata': {
            ...
            'table': 'Table 1: Direct Loan Programs - Budget Formulation Estimates',
...
def parse_table1(excel_path: str, sheet_name: str = 'Table 1',
                 engine: str = None) -> dict:
    """Parse Table 1 from the Federal Credit Supplement Excel file."""
    kwargs = {'header': None}
    if engine:
...
def main():
    ...
    output_path = project_root / 'data' / 'processed' / 'table1_direct_loans.json'

    data = parse_table1(str(excel_path))

    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, 'w') as f:
        json.dump(data, f, indent=2)

    print(f"Parsed {len(data['programs'])} direct loan programs")
